...
function main()
	...
	SetEntityCoords(player, 1365.9979248047,4358.1567382813,44.500373840332-1, 0.0,0.0,0.0, false)
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					...
					if Config.progBar then
						exports['av_progressbar']:startUI(1000, "Tager benzin dunken op...")
					end
		...
		while dodo do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					...
					if Config.progBar then
						exports['av_progressbar']:startUI(20000, "Tanker flyet...")
					end
					Citizen.Wait(20000)
					if Config.useMythic then
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					exports['mythic_notify']:SendAlert('error', 'Annullere')
				end
		...
		while delivering do
			...
			if disttocoord <= 20 then
				...
				if IsControlJustPressed(1, 51) then
					delivering = false
					if Config.progBar then
						exports['av_progressbar']:startUI(Config.pickupTime, "Henter leveringen...")
					end
		...
		while hangar do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					...
					FreezeEntityPosition(airplane, true)
					if Config.progBar then
						exports['av_progressbar']:startUI(2000, "Efterlader nøglerne...")
					end
					Citizen.Wait(2000)
